...
public class Processor_Sketch extends PApplet {
...
public void draw(){  //same as loop in arduino
 ...
 if(ALL_PR_OFF==true) {byte_2 &= ~(30);} //this will clear all the bits in position 9-11.
  ...
  while (port.available() > 0) {
    id_byte = port.read();
    char byte_value =PApplet.parseChar(id_byte);
    if (id_byte == 99) {
      inByte_3 = port.read();
      print("Got a c");
      print(" - ");
...
//so whe you press any button, it sends perticular char over serial port
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#666666", "--stop-color=#cccccc", "Processor_Sketch" };
    if (passedArgs != null) {
